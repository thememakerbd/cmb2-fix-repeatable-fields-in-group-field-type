...
(function(window, document, $, cmb, undefined){
	...
	cmb.increment_last = function( v, i ) {
	...
	cmb.elReplacements = function( $newInput, prevNum, group ) {
		...
		if ( group ) {
			...
			if ( oldFor ) {
				attrs = { 'for' : cmb.increment_last( oldFor, cmb.idNumber ) };
			} else {
				var oldName = $newInput.attr( 'name' );
				console.log(oldName);
				// Replace 'name' attribute key
				var newName = oldName ? cmb.increment_last( oldName, cmb.idNumber ) : '';
				console.log(newName);
				oldID       = $newInput.attr( 'id' );
				newID       = oldID ? cmb.increment_last( oldID, cmb.idNumber ) : '';
				attrs       = {
					id: newID,
					name: newName,
					// value: '',
